...
var yPhysicsObject = function(pPosition, pWeight) {
	...
	yPhysicsObject.prototype.update = function() {
		...
		that.lastCollisions = that.collisions;
		that.collisions = [];
	}

	/**
	 * @method Checks for collision with another physModel
	 */
	...
	/**
	 * @method Draws physics model for debugging purposes
	 */
...
var yGameState = function(){
	...
	that.layers.push(new yLayer());
	
	/**
	 * @method Updates game state
	 */
...
var yLayer = function(){
	...
	ent.physModel = new yPhysicsObject(new yVector(30, 50), 0.8);
	...
	/**
	 * @method Updates layer
	 */
	yLayer.prototype.update = function(){
		var that = this;
		if(input.isAreaPressed(new Area2(new yVector(0, 0), new yVector(canvas.width/4, canvas.height)))){
			that.entities[0].physModel.velocity.x -= 5000*delta;
		}
		if(input.isAreaPressed(new Area2(new yVector(canvas.width/4, 0), new yVector(canvas.width/2, canvas.height)))){
			that.entities[0].physModel.velocity.x += 5000*delta;
		}
		if(input.isAreaPressed(new Area2(new yVector(canvas.width/2, 0), new yVector(canvas.width, canvas.height)))){
	    ...
	    if(input.isDown(input.SPACE) && !input.lastKeyState[input.SPACE]){
	    	if(that.entities[0].physModel.collisions.contains(that.ground)){
	    		that.entities[0].physModel.velocity.y = 200000;
	    	}
		}
	    ...
	    if(input.isDown(input.LEFT)){
	    	keys += 1;
			that.entities[0].physModel.acceleration.x -= 10000*delta;
		}
...
var yInputManager = function() {
	...
	yInputManager.prototype.isDown = function(keyCode) {
		return that.keyState[keyCode];
	};

	yInputManager.prototype.isUp = function(keyCode) {
		return !that.keyState[keyCode];
	};

	yInputManager.prototype.isReleased = function(keyCode) {
		return (!that.keyState[keyCode] && that.lastKeyState[keyCode]);
	};
...
var yTimer = function(pDuration){
	...
	/**
	 * @method Updates remaining time and checks if time is up
	 */
